...
def send_alert(alert_data):
    ...
    subject = os.getenv('CustomerID', '') + " - Missing/Stopped Site 24x7 Agent Check"
    message = "Missing/Stopped Site 24x7 Agent Check Results: \n\n" + alert_data
...
def check_trend_agent_windows():
    ...
    command = "$svc = Get-Service 'Site24x7 Windows Agent' -ErrorAction SilentlyContinue;$svc.Status"
    ...
    if (len(instances_with_wrong_trend_status) > 0):
        ...
        result_str = "Following " + os_family + " instances have missing or stopped Site 24x7 Agent:\n\n"
...
def check_trend_agent_linux():
    ...
    instances = get_instances(os_family)
    command = 'service site24x7monagent status | grep up | wc -l'
