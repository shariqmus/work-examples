...
def send_alert(alert_data):
    ...
    subject = os.getenv('CustomerID', '') + " - Missing/Stopped awslogs/ec2config Agent Check"
    message = "Missing/Stopped awslogs/ec2config Agent Check Results: \n\n" + alert_data
...
def check_windows():
    ...
    # command = "$svc = Get-Service 'Ec2Config' -ErrorAction SilentlyContinue;$svc.Status" # Old command not working for Windows 2016
    command = "$Ver = [System.Environment]::OSVersion.Version.Major;If ($Ver -eq 6) { $svc = Get-Service 'Ec2Config' -ErrorAction SilentlyContinue;$svc.Status; }If ($Ver -eq 10) { $svc = Get-Service 'AmazonSSMAgent' -ErrorAction SilentlyContinue;$svc.Status; }"

    instances_with_wrong_agent_status = []
    ...
    for instance in instances:
        ...
        if (agent_status[0:7] != "Running"):
            instances_with_wrong_agent_status.append(instance)

    result_str = ""

    if (len(instances_with_wrong_agent_status) > 0):

        result_str = "Following " + os_family + " instances have missing or stopped ec2config Agent:\n\n"

        for instance in instances_with_wrong_agent_status:
...
def check_linux():
    ...
    instances = get_instances(os_family)
    command = 'service awslogs status | grep running | head -5 | wc -l'
    ...
    if (len(instances_with_wrong_agent_status) > 0):
        ...
        result_str = "Following " + os_family + " instances have missing or stopped awslogs Agent:\n\n"
...
def lambda_handler(event, context):
    result_lin = check_linux()
    result_win = check_windows()
